...
test.describe('Species list page basic functionality', () => {
  ...
  test('pack loads with correct species count', async ({ page }) => {
    ...
    // Check for species count text - should show "73 species"
    const speciesCountText = page.getByText('73 species');
...
test.describe('Search bar', () => {
  ...
  test('clearing search restores full species list', async ({ page }) => {
    ...
    await searchBar.fill('oak');
    await page.waitForTimeout(400);
    // When filtered, count shows "N of 73 species" — "73 species" exact is not present
    await expect(page.getByText('73 species', { exact: true })).not.toBeVisible();

    await searchBar.clear();
    await page.waitForTimeout(400);
    await expect(page.getByText('73 species', { exact: true })).toBeVisible();
  });
...
test.describe('Quick filter bar', () => {
  ...
  test('habitat chip filters species list', async ({ page }) => {
    ...
    // Count should no longer be "73 species" (some species filtered out)
    await expect(page.getByText('73 species', { exact: true })).not.toBeVisible();
